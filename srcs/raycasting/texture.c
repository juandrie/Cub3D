...
/*   Updated: 2024/06/03 19:19:13 by juandrie         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
double	calculate_wall_x(t_data *data)
{
	if (data->ray->side == 0)
	{
		data->map->wall_x = data->player->pos.y + data->ray->perpwalldist \
		* data->ray->ray_dir.y;
	}
	else
	{
		data->map->wall_x = data->player->pos.x + data->ray->perpwalldist \
		* data->ray->ray_dir.x;
	}
